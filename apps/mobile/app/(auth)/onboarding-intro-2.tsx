...
const background = require('../../assets/images/figma-onboarding-intro-2.png');
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gestureState) => {
        if (gestureState.dx < -SWIPE_THRESHOLD) {
          router.replace('/(auth)/onboarding-intro-3');
    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
      ...
      <ImageBackground source={background} style={styles.background} resizeMode="stretch">
        <View style={styles.overlay} {...panResponder.panHandlers}>
          <TouchableOpacity style={styles.backHitbox} onPress={() => router.replace('/(auth)/onboarding-intro-1')} />
          <TouchableOpacity style={styles.primaryButtonHitbox} onPress={() => router.replace('/(auth)/onboarding-intro-3')} />
        </View>
      </ImageBackground>
    </SafeAreaView>
  );
